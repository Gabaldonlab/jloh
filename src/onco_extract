...
p = ap.ArgumentParser(description="Detect LOH blocks in tumoral setups from BAM(s), VCF(s) and REF as inputs.")
...
def check_conditions(args):
    ...
    """
    Determine if all input files are present, if not error will raise
    """
...
def extract_variants(vcf_input, vcf_output, min_af, max_af, dir, name):
    ...
    '''
    Function to extract the variants from the VCF file and check if:
    1.Is a real SNP
...
def create_subset_bam_file(bam, tmp_bams, chrom):
    ...
    #Useful to determine zygosity and coverage of LOH candidates, appart from other functions
...
def split_bam_by_chromosome(bam, genome_file, tmp_bams, args):
    ...
    Chroms=Chroms["Chrom"].tolist()[:25] #chr1-22 + M,X and Y
...
def hetero_and_homo_snp_densities(ref,vcf, name):
    '''determine the densitites of individuals regarding their zygosity'''
...
def filter_by_snp_count(args,bt, discard, min_snps_kbp):
    #modified to allow the use of PD dataframes
    df = pd.DataFrame([str(row).rstrip("\n\r\b").split("\t") for row in bt], columns=['chr', 'start', 'end', 'count']) #divide into columns adding their names
    df[['start', 'end', 'count']] = df[['start', 'end', 'count']].apply(pd.to_numeric) #to consider it as a number instead of str
...
def snps_to_bed_blocks(args, het_snps_vcf, homo_snps_vcf_ALT, homo_snps_vcf_REF, genome_file):
    ...
    '''
    29/05/2024
    Debugging method to avoid raising errors if no information is found in a specific sample
    '''
...
def add_allele_column(args, Het_blocks, Homo_blocks_REF, Homo_blocks_ALT):
    ...
    '''This will be used for trimming with heterozygous regions + adding allele type'''
...
def chromosome_sort_key(chromosome):
    '''It was needed to avoid sorting the files lexicographically, so this function returns a
    tupple (True/False, chromosome number or letter). The python function will be
    able to compare efficiently, comparing first True (int) and then False (strings as M, X and Y)'''

    if chromosome.startswith('chr'): #this will get the number or M,Y,X
        chromosome = chromosome[3:]
    try: #to order numerically chromosomes
...
def assess_coverage(args, Chrom_bams, genome_file, Homo_blocks, sample):
    ...
    bt = BedTool(LOH_line if LOH_line else [""])  # Ensure bt is created even if LOH_line is empty (debug)
...
def find_relevant_regions(args,LOH):
    #add BED file of regions to find if they intersect with LOH regions
...
def remove_het_snps_in_output(LOH, args):
    #Remove blocks where:
    # 1. Number of hetero snps is equal to number homo snps
    # 2. Situations where only 1 homo SNP is found (remove errors while calling LOH blocks)
    filtered_LOH = []  
...
def remove_LOH_control(control, tumor,fraction):
    '''
    Added to ensure LOH blocks from the tumoral samples were not found as homozygous clusters in control sample
    '''
    control = BedTool(control)
    tumor = BedTool(tumor)
    inter = tumor.intersect(control, v=True, f=fraction) #can be modified by the user
    return inter

def run_in_default_mode(args, tmp_bams, tmp_bams_tumor):

    #paired samples within the same individual
    #MODULE PROCESSING
    pid = os.getpid()
...
def run_in_single_mode(args, tmp_bams, tmp_bams_control):
    ...
    #no control sample is given to JLOH
    #MODULE PROCESSING
    pid = os.getpid()
    process = psutil.Process(pid)
    ...
    #create the BED file (0-based) with coordinates and a TSV (1-based) file with complete information
...
if __name__ == "__main__":
    ...
    # This generates a second unique key for the new temp directory, essential for default mode
